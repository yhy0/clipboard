...
# 查找 DerivedData 中的应用
DERIVED_DATA=$(xcodebuild -scheme "$SCHEME" -configuration "$CONFIGURATION" -showBuildSettings 2>/dev/null | grep " BUILT_PRODUCTS_DIR" | sed 's/.*= //')
...
echo "   3. 启动测试服务器: ./test_sparkle_local.sh"
echo "   4. 运行应用并点击「检查更新」"
echo ""
